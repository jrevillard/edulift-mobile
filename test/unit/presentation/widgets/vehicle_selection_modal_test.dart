...
void main() {
  group('VehicleSelectionModal - Solution D Implementation', () {
    const filePath = 'lib/features/schedule/presentation/widgets/vehicle_selection_modal.dart';
    ...
    test('Should have snap positions configured correctly', () {
      ...
      expect(content.contains('snap: true'), isTrue,
        reason: 'DraggableScrollableSheet should have snap enabled');

      expect(content.contains('snapSizes: const [0.4, 0.6, 0.95]'), isTrue,
        reason: 'Snap positions should be [0.4, 0.6, 0.95] for mobile optimization');
    });
    ...
    test('Should have removed dead code methods', () {
      ...
      expect(content.contains('_buildMultipleTimeSlotsContent'), isFalse,
        reason: 'Dead code _buildMultipleTimeSlotsContent should be removed');

      // Verify _buildTimeSlotSection was removed
      expect(content.contains('_buildTimeSlotSection'), isFalse,
    ...
    test('Should use _buildEnhancedTimeSlotList for multiple slots', () {
      ...
      expect(content.contains('_buildEnhancedTimeSlotList'), isTrue,
        reason: 'New method _buildEnhancedTimeSlotList should exist');

      // Verify it uses ExpansionTile
      expect(content.contains('ExpansionTile'), isTrue,
      ...
      final enhancedMethodEnd = content.indexOf('Widget _buildSingleSlotContent', enhancedMethodStart);
      final enhancedMethodBody = content.substring(enhancedMethodStart, enhancedMethodEnd);

      expect(enhancedMethodBody.contains('_buildSingleSlotContent'), isTrue,
    ...
    test('Should use i18n for all user-facing strings', () {
      ...
      expect(methodBody.contains("'vehicle'"), isFalse,
        reason: 'Should not have hardcoded "vehicle" string');

      expect(methodBody.contains("'vehicles'"), isFalse,
        reason: 'Should not have hardcoded "vehicles" string');

      expect(methodBody.contains("'No vehicles'"), isFalse,
        reason: 'Should not have hardcoded "No vehicles" string');

      expect(methodBody.contains("'Expand'"), isFalse,
        reason: 'Should not have hardcoded "Expand" string');

      // Verify AppLocalizations is used
      expect(methodBody.contains('AppLocalizations.of(context)'), isTrue,
    ...
    test('Should have ExpansionTile with proper touch targets (WCAG AAA)', () {
      ...
      expect(
        content.contains('tilePadding:') && content.contains('EdgeInsets.symmetric'),
        isTrue,
        reason: 'ExpansionTile should have appropriate padding for touch targets');

      // Verify leading icon is 48x48
      expect(content.contains('width: 48'), isTrue,
        reason: 'Leading icon should be 48px wide (WCAG minimum)');

      expect(content.contains('height: 48'), isTrue,
    ...
    test('Should have reduced total lines of code (dead code removed)', () {
      ...
      expect(lines.length, lessThan(1400),
