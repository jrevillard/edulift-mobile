...
void main() {
  group('AuthService 422 Surgical Fix Tests', () {
    ...
    test(
      'SURGICAL FIX: 422 magic link preserves backend message "name is required for new users"',
      () async {
        // ARRANGE
        ...
        // Setup PKCE storage
        when(
          mockAuthDatasource.storePKCEVerifier(any),
        ).thenAnswer((_) async => const Result.ok(null));
        when(
          mockAuthDatasource.storeMagicLinkEmail(email),
        ).thenAnswer((_) async => const Result.ok(null));
        ...
        const apiException = ApiException(
          ...
          statusCode: 422,
          details: {'error': backendMessage, 'code': 'VALIDATION_ERROR'},
        );
        ...
        expect(
          validationFailure.details!['original_message'],
          equals(backendMessage),
        );
        expect(
          validationFailure.details!['error_source'],
          equals('auth_magic_link'),
        );
        expect(
          validationFailure.details!['preserve_backend_message'],
          equals(true),
        );
    ...
    test(
      'SURGICAL FIX: 422 preserves message from details when direct message is empty',
      () async {
        // ARRANGE
        const email = 'new.user@example.com';
        const backendMessage =
            'name field is missing for new user registration';
    ...
    test(
      'SURGICAL FIX: Fallback message works when no message available',
      () async {
        // ARRANGE
