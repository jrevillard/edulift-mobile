...
void main() {
  group('Router Magic Link Fix Verification', () {
    ...
    test('router condition logic for magic link routes', () {
      ...
      bool shouldRedirectToLogin(String location, bool isAuthenticated, bool hasPendingEmail) {
      ...
      const hasPendingEmail = false; // This is the key scenario - no pending email

      // SCENARIO 1: Magic link waiting page without pending email - SHOULD redirect
      expect(
        shouldRedirectToLogin('/auth/login/magic-link', isAuthenticated, hasPendingEmail),
        true,
      ...
      expect(
        shouldRedirectToLogin('/auth/verify?token=abc123', isAuthenticated, hasPendingEmail),
        false,
      ...
      expect(
        shouldRedirectToLogin('/auth/verify?token=abc123&email=test@example.com', isAuthenticated, hasPendingEmail),
        false,
    ...
    test('E2E test failure scenarios are now fixed', () {
      ...
      const waitingPageLocation = '/auth/login/magic-link?email=test@example.com';
      const verificationLocation = '/auth/verify?token=test-token&email=test@example.com';
      const isAuthenticated = false;
      const hasPendingEmail = false; // This would be null in real failure scenario

      // OLD LOGIC (broken): would redirect away from waiting page even with email in URL
      final oldLogicWouldRedirectWaiting = waitingPageLocation.startsWith(AppRoutes.magicLink) &&
                                         !isAuthenticated &&
                                         !hasPendingEmail;

      // NEW LOGIC (fixed): same for waiting page (this is correct behavior)
      final newLogicWouldRedirectWaiting = waitingPageLocation.startsWith(AppRoutes.magicLink) &&
                                         !waitingPageLocation.startsWith(AppRoutes.verifyMagicLink) &&
                                         !isAuthenticated &&
                                         !hasPendingEmail;

      // For verification page - neither old nor new logic should redirect
      final oldLogicWouldRedirectVerify = verificationLocation.startsWith(AppRoutes.magicLink) &&
                                        !isAuthenticated &&
                                        !hasPendingEmail;

      final newLogicWouldRedirectVerify = verificationLocation.startsWith(AppRoutes.magicLink) &&
      ...
      expect(oldLogicWouldRedirectWaiting, true, reason: 'Waiting page without pending email should redirect');
      expect(newLogicWouldRedirectWaiting, true, reason: 'Waiting page without pending email should redirect');

      // Neither old nor new logic should redirect from verification page (verification doesn't start with magic-link)
      expect(oldLogicWouldRedirectVerify, false, reason: 'Verification page should never redirect (old logic)');
      expect(newLogicWouldRedirectVerify, false, reason: 'Verification page should never redirect (new logic)');
