...
void main() {
  group('Golden Test Network Isolation Architecture Rules', () {
    ...
    test('All golden test files must import network_mocking.dart OR not use provider overrides', () {
      ...
      for (final file in goldenTestFiles) {
        ...
        final usesProviderOverrides = content.contains('providerOverrides:') ||
            content.contains('providerOverrides =');

        if (usesProviderOverrides) {
          // If it uses provider overrides, it must import network_mocking
          final hasNetworkMockingImport =
              content.contains("import '../../support/network_mocking.dart'");
    ...
    test('All golden test files with provider overrides must call getAllNetworkMockOverrides()', () {
      final violations = <String>[];
      ...
      for (final file in goldenTestFiles) {
        ...
        if (usesProviderOverrides) {
          ...
          final callsGetAllNetworkMockOverrides =
              content.contains('getAllNetworkMockOverrides()');
